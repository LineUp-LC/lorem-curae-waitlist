...
// ============================================================================
// GET /api/admin/search-waitlist
// ============================================================================
//
// Flexible search endpoint for the waitlist with multiple optional filters.
...
//   email (string): Partial match using ILIKE
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (!adminSecret) {
      console.error('[search-waitlist] Missing ADMIN_SECRET');
      return res.status(500).json({ error: 'Server misconfigured: missing ADMIN_SECRET' });
    }

    const supabaseUrl = process.env.SUPABASE_URL;
    if (!supabaseUrl) {
      console.error('[search-waitlist] Missing SUPABASE_URL');
      return res.status(500).json({ error: 'Server configuration error' });
    }

    const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!serviceRoleKey) {
      console.error('[search-waitlist] Missing SUPABASE_SERVICE_ROLE_KEY');
      return res.status(500).json({ error: 'Server configuration error' });
    }
    ...
    if (!authHeader || authHeader !== `Bearer ${adminSecret}`) {
      console.warn('[search-waitlist] Unauthorized request attempt');
    ...
    if (emailParam !== undefined) {
      if (typeof emailParam !== 'string') {
        return res.status(400).json({ error: 'email must be a single string value' });
      }
    ...
    // Apply email filter (partial match with ILIKE)
    if (emailFilter) {
    ...
    if (error) {
      ...
      if (error.code === '42P01' || error.message?.includes('does not exist')) {
        console.log('[search-waitlist] Table does not exist, returning empty results');
        return res.status(200).json({ results: [] });
      }
      console.error('[search-waitlist] Query error:', error);
      return res.status(500).json({ error: 'Failed to search waitlist' });
    }
    ...
    console.error('[search-waitlist] Unexpected error:', error);

...
async function sendEmail(to: string, templateKey: string): Promise<void> {
  ...
  console.log(`[sendEmail] Sending template "${templateKey}" to ${to}`);
  ...
  if (!response.ok) {
    ...
    console.error(`[sendEmail] Failed to send email: ${response.statusText}`, errorText);
    throw new Error(`Failed to send email: ${response.statusText}`);
  }

  console.log(`[sendEmail] Successfully sent template "${templateKey}" to ${to}`);
}
...
async function sendWaitlistSignupEmail(email: string): Promise<void> {
  console.log(`[sendWaitlistSignupEmail] Called with email: ${email}`);

  const waitlist = await fetchWaitlistRecord(email);
  console.log(`[sendWaitlistSignupEmail] Waitlist record:`, waitlist);

  if (!waitlist) {
    console.log(`[sendWaitlistSignupEmail] No waitlist record, using default template`);
    await sendEmail(email, 'waitlist_signup');
    return;
  }

  const role = determineUserRole(waitlist);
  console.log(`[sendWaitlistSignupEmail] Determined role: ${role}`);
  ...
  if (role === 'founding_member') {
    if (waitlist.wants_tester_access && waitlist.is_creator) {
      console.log(`[sendWaitlistSignupEmail] Using founding_member_tester_creator_signup`);
      await sendEmail(email, 'founding_member_tester_creator_signup');
      return;
    }
    if (waitlist.wants_tester_access && !waitlist.is_creator) {
      console.log(`[sendWaitlistSignupEmail] Using founding_member_tester_consumer_signup`);
      await sendEmail(email, 'founding_member_tester_consumer_signup');
      return;
    }
    if (waitlist.is_creator) {
      console.log(`[sendWaitlistSignupEmail] Using founding_member_creator_signup`);
  ...
  console.log(`[sendWaitlistSignupEmail] Using template: ${templateKey}`);
...
async function sendMagicLinkEmail(email: string): Promise<void> {
  ...
  const waitlist = await fetchWaitlistRecord(email);
  console.log(`[sendMagicLinkEmail] Waitlist record:`, waitlist);

  if (!waitlist) {
    console.log(`[sendMagicLinkEmail] No waitlist record, using default template`);
    await sendEmail(email, 'non_tester_login');
    return;
  }

  const role = determineUserRole(waitlist);
  console.log(`[sendMagicLinkEmail] Determined role: ${role}`);

  // Founding members have special handling based on tester status and creator status
  if (role === 'founding_member') {
    if (waitlist.wants_tester_access && waitlist.is_creator) {
      console.log(`[sendMagicLinkEmail] Using founding_member_tester_creator_login`);
      await sendEmail(email, 'founding_member_tester_creator_login');
      return;
    }
    if (waitlist.wants_tester_access && !waitlist.is_creator) {
      console.log(`[sendMagicLinkEmail] Using founding_member_tester_consumer_login`);
      await sendEmail(email, 'founding_member_tester_consumer_login');
      return;
    }
    if (waitlist.is_creator) {
      console.log(`[sendMagicLinkEmail] Using founding_member_creator_login`);
  ...
  console.log(`[sendMagicLinkEmail] Using template: ${templateKey}`);
  await sendEmail(email, templateKey);
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  console.log(`[send-role-based-email] Received ${req.method} request`);
  ...
  console.log(`[send-role-based-email] Request body:`, { email, type });

  if (!email || !type) {
    console.log(`[send-role-based-email] Missing email or type`);
    return res.status(400).json({ error: 'Email and type are required' });
  }

  try {
    if (type === 'signup') {
      await sendWaitlistSignupEmail(email);
    } else if (type === 'login') {
      await sendMagicLinkEmail(email);
    } else {
      console.log(`[send-role-based-email] Invalid type: ${type}`);
      return res.status(400).json({ error: 'Invalid type. Use "signup" or "login"' });
    }

    console.log(`[send-role-based-email] Successfully sent ${type} email to ${email}`);
    return res.status(200).json({ success: true });
  } catch (error) {
    console.error('[send-role-based-email] Error:', error);
